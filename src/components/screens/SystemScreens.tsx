...
export const SyncCenterScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-5 pb-8 animate-in fade-in duration-300">
      <div className="p-5 rounded-2xl bg-slate-900 border border-slate-700/70 shadow-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className={`w-2.5 h-2.5 rounded-full ${db.isOnline ? 'bg-emerald-400 shadow-[0_0_8px_#34d399]' : 'bg-rose-500'}`} />
            <span className="font-bold text-white text-sm">{db.isOnline ? 'ONLINE' : 'OFFLINE'}</span>
          </div>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800">
          <span className="text-xs text-slate-400">Outbox Queue Items</span>
          <h3 className="font-mono font-bold text-white text-lg mt-1">{queue.length} Pending</h3>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800">
          <span className="text-xs text-slate-400">Last Successful Sync</span>
          <h3 className="font-bold text-cyan-300 text-sm mt-1">{db.lastSyncTime}</h3>
        </div>

        <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800">
          <span className="text-xs text-slate-400">Cloud Target</span>
          <h3 className="font-bold text-white text-sm mt-1">PostgreSQL Multi-Tenant</h3>
        </div>
      </div>
      ...
      <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-700/70 shadow-2xl overflow-hidden">
        ...
        {queue.length === 0 ? (
          <div className="py-12 text-center text-slate-400 text-xs">
            ...
            <p className="font-semibold text-slate-300">All local changes are fully synchronized.</p>
            <p className="text-slate-500 mt-0.5">Local SQLite database is in sync with Supabase cloud PostgreSQL.</p>
          </div>
        ) : (
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {queue.map(item => (
                <tr key={item.id}>
                  <td className="py-2.5 font-mono text-cyan-400">{item.id.slice(0, 16)}...</td>
                  <td className="py-2.5 font-semibold text-white uppercase text-[10px]">{item.entityType}</td>
                  <td className="py-2.5">
                    <span className="px-2 py-0.5 rounded bg-cyan-950/60 text-cyan-300 font-mono text-[10px] border border-cyan-800/40">
                      {item.operation}
                    </span>
                  </td>
                  <td className="py-2.5 text-slate-400">{item.deviceId}</td>
                  <td className="py-2.5 text-slate-400">{item.createdAt.slice(11, 19)}</td>
                  <td className="py-2.5">
                    <span
          ...
          <tbody className="divide-y divide-slate-800/60">
            {db.auditLogs.map(log => (
              <tr key={log.id}>
                ...
                <td className="py-2.5 text-slate-300 text-xs">{log.details}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="space-y-5 pb-8 animate-in fade-in duration-300">
      <div className="p-5 rounded-2xl bg-slate-900 border border-slate-700/70 shadow-xl flex justify-between items-center">
        ...
        <button
          ...
          className="px-4 py-2 bg-gradient-to-r from-cyan-500 to-blue-600 text-white font-bold rounded-xl shadow-lg flex items-center gap-2 text-xs"
        >
      ...
      {downloadSuccess && (
        <div className="p-4 rounded-xl bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs flex items-center gap-2">
          ...
          Backup created and saved to local disk. Encrypted with AES-256 local key.
        </div>
      )}

      <div className="p-6 rounded-2xl bg-slate-900/90 border border-slate-700/70 shadow-2xl space-y-4">
        <h3 className="font-bold text-sm text-white">Automated Snapshot History</h3>
        <div className="space-y-2">
          {[
            ...
            <div key={i} className="flex justify-between items-center p-3 rounded-xl bg-slate-950/60 text-xs border border-slate-800">
              <div className="flex items-center gap-3">
                <Database className="w-4 h-4 text-cyan-400" />
                <div>
                  <span className="font-mono text-white font-bold block">{b.name}</span>
                  <span className="text-slate-400 text-[11px]">{b.size} • {b.date}</span>
                </div>
              </div>
    ...
    <div className="space-y-5 pb-8 animate-in fade-in duration-300">
      ...
      <div className="p-6 rounded-2xl bg-slate-900/90 border border-slate-700/70 shadow-2xl space-y-4">
        <h2 className="text-sm font-bold text-white uppercase tracking-wider">
          Appearance & Theme System (Mandatory First-Class Requirement)
        </h2>
        ...
        <div className="grid grid-cols-3 gap-4 pt-2">
          ...
          >
            ...
            <span className="text-xs font-bold">🌙 Dark Theme (Master Reference)</span>
          </button>
        ...
        <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-3 mt-4">
          <span className="text-[11px] font-bold text-slate-400 uppercase">Live Token Preview</span>
          <div className="flex gap-3">
            <div className="p-3 rounded-lg bg-cyan-500/20 border border-cyan-500/30 text-cyan-300 text-xs font-semibold">
              Primary Accent Pill
            </div>
            <div className="p-3 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-xs font-semibold">
              Success State Badge
            </div>
            <div className="p-3 rounded-lg bg-pink-500/20 border border-pink-500/30 text-pink-300 text-xs font-semibold">
              Financial Highlight
            </div>
          </div>
        </div>
  ...
  return (
    <div className="space-y-5 pb-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {diagnostics.map((d, i) => (
          <div key={i} className="p-4 rounded-xl bg-slate-900/90 border border-slate-700/70 shadow-xl flex items-start justify-between">
            <div>
              <h4 className="font-bold text-sm text-white">{d.name}</h4>
              <p className="text-xs text-slate-400 mt-0.5">{d.details || d.version}</p>
            </div>
