...
export const SeatTransferScreen: React.FC<{
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 pb-8 animate-in fade-in duration-300">
      ...
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-700/70 shadow-xl space-y-5 text-xs">
        {/* Student Selector */}
        <div>
          <label className="block text-slate-600 dark:text-slate-400 mb-1 font-medium">Select Student to Transfer</label>
          <select
        ...
        <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800">
          <div className="space-y-1">
            ...
            <div className="p-3 rounded-lg bg-rose-500/10 border border-rose-500/20 text-rose-700 dark:text-rose-300">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">Room A - Main Reading Hall</span>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-cyan-700 dark:text-cyan-300">
              <span className="text-lg font-mono font-bold block">Seat {toSeatNumber}</span>
              <span className="text-xs text-slate-500 dark:text-slate-400">{targetRoom.name}</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-slate-600 dark:text-slate-400 mb-1">New Seat Number</label>
            <input
              type="text"
              value={toSeatNumber}
              onChange={e => setToSeatNumber(e.target.value.toUpperCase())}
              className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-cyan-600 dark:text-cyan-300 font-mono font-bold uppercase"
            />
          </div>
        </div>

        <div>
          <label className="block text-slate-600 dark:text-slate-400 mb-1">Transfer Reason / Remarks</label>
          <input
            type="text"
            value={reason}
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-700/70 shadow-xl flex justify-between items-center">
        ...
        <div className="flex gap-4 text-xs">
          ...
          <span className="flex items-center gap-1.5 text-blue-600 dark:text-blue-400 font-bold">
            <span className="w-2.5 h-2.5 rounded-full bg-blue-500" /> {occupiedCount} Occupied
          </span>
        </div>
      </div>

      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-700/70 shadow-xl">
            ...
            return (
              <div
                ...
                className={`p-3 rounded-xl border text-center cursor-pointer transition flex flex-col items-center justify-center ${
                  isOcc
                    ? 'bg-purple-50 dark:bg-purple-950/30 border-purple-200 dark:border-purple-500/40 text-purple-700 dark:text-purple-200 hover:border-purple-400'
                    : 'bg-emerald-50 dark:bg-emerald-950/20 border-emerald-200 dark:border-emerald-500/30 text-emerald-700 dark:text-emerald-300 hover:border-emerald-400'
                }`}
              >
        ...
        <div className="pt-6 mt-6 border-t border-slate-100 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400 flex justify-between">
  ...
  return (
    <div className="space-y-5 pb-8 animate-in fade-in duration-300">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
        {/* Entry Form (4 cols) */}
        <div className="md:col-span-4 p-5 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-700/70 shadow-xl space-y-3 text-xs">
          <h3 className="font-bold text-sm text-slate-900 dark:text-white">New Visitor Entry</h3>
          <div>
            <label className="block text-slate-600 dark:text-slate-400 mb-1">Visitor Full Name</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-slate-600 dark:text-slate-400 mb-1">Purpose of Visit</label>
            <input
              type="text"
              value={purpose}
          ...
          <button
            onClick={handleAdd}
            className="w-full py-2 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white font-bold rounded-xl mt-2 transition shadow"
          >
            Check In Visitor
          </button>
        </div>
        ...
        <div className="md:col-span-8 p-5 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-700/70 shadow-xl">
          <h3 className="font-bold text-sm text-slate-900 dark:text-white mb-3">Today's Visitors</h3>
          <table className="w-full text-xs text-left">
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {db.visitors.map(v => (
                <tr key={v.id}>
                  <td className="py-2.5 font-semibold text-slate-900 dark:text-white">{v.name}</td>
                  <td className="py-2.5 text-slate-500 dark:text-slate-400">{v.mobile}</td>
                  <td className="py-2.5 text-slate-700 dark:text-slate-300">{v.purpose}</td>
                  <td className="py-2.5 text-slate-500 dark:text-slate-400">{v.entryTime}</td>
                  <td className="py-2.5">
                    <span
                  ...
                  <td className="py-2.5 text-right">
                    {v.status === 'Inside' && (
                      <button
                        ...
                        className="px-2 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-rose-600 dark:text-rose-300 text-[10px] rounded font-semibold"
                      >
                        Exit
                      </button>
                    )}
...
export const ComplaintsScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-5 pb-8 animate-in fade-in duration-300">
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-slate-900/90 border border-slate-200/80 dark:border-slate-700/70 shadow-xl space-y-4">
        {complaints.map(cmp => (
          <div key={cmp.id} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 space-y-2 text-xs">
            <div className="flex justify-between items-start">
              <div>
                <span className="font-mono text-cyan-600 dark:text-cyan-400 font-bold mr-2">{cmp.complaintNo}</span>
                <span className="font-bold text-slate-900 dark:text-white text-sm">{cmp.studentName}</span>
                <span className="text-slate-500 dark:text-slate-400 text-[11px] ml-2 font-medium">Category: {cmp.category}</span>
              </div>
              <span
            ...
            <p className="text-slate-700 dark:text-slate-300">{cmp.description}</p>
            {cmp.resolution && (
              <p className="text-emerald-700 dark:text-emerald-400 text-[11px] bg-emerald-50 dark:bg-emerald-950/30 p-2 rounded border border-emerald-200 dark:border-emerald-800/40">
                Resolution: {cmp.resolution}
              </p>
            )}
            <div className="flex justify-between text-slate-400 dark:text-slate-500 text-[10px] pt-1">
...
// ----------------------------------------------------
// Screen 32: Notice Board (Delegated to full NoticesScreen component)
// ----------------------------------------------------
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
            {db.staff.map(s => (
              <tr key={s.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                ...
                <td className="py-3 text-cyan-600 dark:text-cyan-300 font-medium">{s.role}</td>
                <td className="py-3 text-slate-500 dark:text-slate-400">{s.mobile}</td>
                <td className="py-3 text-slate-700 dark:text-slate-300">{s.shift}</td>
                <td className="py-3 font-mono font-bold text-emerald-600 dark:text-emerald-400">₹{s.salary.toLocaleString('en-IN')}</td>
                <td className="py-3">
